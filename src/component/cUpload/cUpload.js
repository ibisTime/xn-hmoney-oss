...
export default class CUpload extends React.Component {
    ...
    isPropsChange(nextProps) {
        const {
            field, isLoaded, token, rules, readonly, isSingle, isImg,
            accept, getFieldValue, hidden, initVal, inline
        } = this.props;
    ...
    // 获取文件上传的属性
    getUploadProps = ({
                          initValue, token, field, readonly = false,
    ...
    render() {
        const {
            field, isLoaded, getFieldDecorator, token, rules, readonly, isSingle,
            isImg, onChange, accept, getFieldValue, label, hidden, initVal, inline
        } = this.props;
