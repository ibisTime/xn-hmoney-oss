...
import {
    ...
    setPageData,
    restore
} from '@redux/finance/offlineRecharge/offlineRecharge-addedit';
...
@DetailWrapper(
    state => state.financeOfflineRechargeAddEdit,
...
class OfflineRechargeAddedit extends React.Component {
    ...
    render() {
        let fields = [{
            ...
            pageCode: '805120',
            keyName: 'userId',
            valueName: '{{realName.DATA}}({{nickname.DATA}})-{{mobile.DATA}}',
            searchName: 'keyword',
            onChange: (v, data) => {
                if (v) {
                    getListUserAccount({userId: v, currency: 'ETH'}).then((d) => {
